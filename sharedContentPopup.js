...
function sharedInit(getCurrentTabUrl, renderStatus, displayResult, displayButtonNote, displayImmediateFailures) {
    try {
        getCurrentTabUrl(function (url) {
            ...
            if (matches.length < 2 && displayImmediateFailures) {
            ...
            if (!supported && displayImmediateFailures) {
                ...
                var buttonNote = sitesInfo[matches[1]].buttonNote;
                if (buttonNote) {
                    displayButtonNote(buttonNote);
                }
            }

            // See if http://www.deslide.clusterfake.net supports this site and show a link if so
            var deslideSupportedFormat = _deslideSites[matches[1]];
            if (!!deslideSupportedFormat) {
                renderStatus('You can view this page on Deslide' + (supported ? ' too' : '') + '! Try the link below!', true);
...
/**
 * Returns sites supported by http://www.deslide.clusterfake.net.
 * 
